...
public class AdHealthPrivacyManager : MonoBehaviour
{
    ...
    [Header("绑定")] [SerializeField] private GameObject privacyObj;
    ...
    [Header("类型")]
    [SerializeField]
    private HealthType type = HealthType.Text;

    #region Image

    [SerializeField, Display(nameof(type), nameof(HealthType.Image))]
    ...
    [SerializeField, Display(nameof(type), nameof(HealthType.Text))]
    private string 著作权人 = "";
    [SerializeField, Display(nameof(type), nameof(HealthType.Text))]
    private string 登记号 = "";
    [SerializeField, Display(nameof(type), nameof(HealthType.Text))]
    private Age age = Age.Twelve;
