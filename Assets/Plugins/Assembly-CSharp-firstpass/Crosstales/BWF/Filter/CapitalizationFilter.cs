...
namespace Crosstales.BWF.Filter
{
	public class CapitalizationFilter : BaseFilter
	{
		...
		public CapitalizationFilter(int capitalizationCharsNumber)
		{
			...
			else
			{
				...
				foreach (Match item in matchCollection)
				{
					foreach (Capture capture in item.Captures)
					{
						...
						text2 = text2.Replace(capture.Value, (!markOnly) ? (prefix + capture.Value.ToLowerInvariant() + postfix) : (prefix + capture.Value + postfix));
					}
				}
			}
			return text2;
