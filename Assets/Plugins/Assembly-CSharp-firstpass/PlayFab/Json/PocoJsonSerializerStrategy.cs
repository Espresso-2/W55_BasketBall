		...
		public virtual object DeserializeObject(object value, Type type)
		{
			...
			if (text != null)
			{
				if (text.Length != 0)
				{
					...
					{
						Uri result2;
						if (Uri.IsWellFormedUriString(text, UriKind.RelativeOrAbsolute) && Uri.TryCreate(text, UriKind.RelativeOrAbsolute, out result2))
						{
							return result2;
						}
						return null;
		...
		protected virtual bool TrySerializeUnknownTypes(object input, out object output)
		{
			...
			foreach (KeyValuePair<MemberInfo, ReflectionUtils.GetDelegate> item in dictionary2)
			{
				if (item.Value != null)
				{
					...
					MapClrMemberNameToJsonFieldName(item.Key, out jsonName, out jsonProp);
