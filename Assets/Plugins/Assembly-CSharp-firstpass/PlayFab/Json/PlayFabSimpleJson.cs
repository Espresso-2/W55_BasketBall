...
namespace PlayFab.Json
{
	...
	public static class PlayFabSimpleJson
	{
		...
		private static string ParseString(string json, ref int index, ref bool success)
		{
			...
			while (!flag && index != json.Length)
			{
				...
				case '\\':
				{
					...
					uint result;
					if (!(success = uint.TryParse(json.Substring(index, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out result)))
					{
						return string.Empty;
					}
		...
		private static object ParseNumber(string json, ref int index, ref bool success)
		{
			...
			{
				double result;
				success = double.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out result);
				result2 = result;
			}
			else if (text.IndexOf("-", StringComparison.OrdinalIgnoreCase) == -1)
			{
				ulong result3;
				success = ulong.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out result3);
				result2 = result3;
			}
			else
			{
				long result4;
				success = long.TryParse(json.Substring(index, length), NumberStyles.Any, CultureInfo.InvariantCulture, out result4);
				result2 = result4;
			}
