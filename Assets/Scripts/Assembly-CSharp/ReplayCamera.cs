...
public class ReplayCamera : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        GameObject RecordCamera = GameObject.Find("RecordCamera");
        if (RecordCamera != null)
        {
            recorder = RecordCamera.GetComponent<Recorder>();
            bballReplayRecorder = RecordCamera.GetComponent<BasketballReplayRecorder>();
        }
        if (recorder != null && recorder.RenderTextureFrames != null)
        {
            ...
            replayBorderRect = new Rect(replayRect.x - replayBorderSize, replayRect.y - replayBorderSize, replayRect.width + replayBorderSize * 2f,
                replayRect.height + replayBorderSize * 2f);
    ...
    private void RequestAndroidWritePermission()
    {
        if (Application.platform == RuntimePlatform.Android &&
            !AndroidPermissionsManager.IsPermissionGranted("android.permission.WRITE_EXTERNAL_STORAGE"))
        {
            AndroidPermissionsManager.RequestPermission(new string[1] { "android.permission.WRITE_EXTERNAL_STORAGE" },
                new AndroidPermissionCallback(delegate { }, delegate { }));
        }
    }*/
    ...
    /*private void OpenAndroidGallery()
    {
        ...
        AndroidJavaObject androidJavaObject2 =
            androidJavaClass2.CallStatic<AndroidJavaObject>("parse", new object[1] { "content://media/internal/images/media" });
